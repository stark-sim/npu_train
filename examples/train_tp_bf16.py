...
Uses bfloat16 for model weights with mixed precision for training.
"""
...
def main():
    ...
    # Setup optimizer - convert to float32 for bfloat16 models
    # Create float32 copies of parameters for optimizer
    if model_dtype in [torch.bfloat16, torch.float16]:
        # For mixed precision, use master weights in float32
        master_params = []
        for p in model.parameters():
            if p.requires_grad:
                master_p = p.float().detach().clone()
                master_p.requires_grad = True
                master_params.append(master_p)
            else:
                master_params.append(None)

        optimizer = torch.optim.AdamW(
            [p for p in master_params if p is not None],
            lr=args.lr,
            betas=(0.9, 0.95),
            eps=1e-8,
        )

        def sync_master_params():
            """Copy model params to master params"""
            for mp, p in zip(master_params, model.parameters()):
                if mp is not None and p.requires_grad:
                    mp.data.copy_(p.data)

        def sync_model_params():
            """Copy master params to model params"""
            for mp, p in zip(master_params, model.parameters()):
                if mp is not None and p.requires_grad:
        ...
        sync_master_params = lambda: None
        sync_model_params = lambda: None
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (input_ids, labels) in enumerate(dataloader):
            ...
            # Sync master params with model params before forward
            ...
            # Copy gradients to master params and step optimizer
            if master_params is not None:
                # Copy gradients from model params to master params
                for mp, p in zip(master_params, model.parameters()):
                    if mp is not None and p.requires_grad and p.grad is not None:
                        if mp.grad is None:
                            mp.grad = mp.data.new(mp.size())
                        mp.grad.copy_(p.grad)

                optimizer.step()
                optimizer.zero_grad()

                # Copy updated master params back to model
                sync_model_params()
            else:
                optimizer.step()
                optimizer.zero_grad()
