...
def main():
        ...
        for batch_idx, (input_ids, labels) in enumerate(dataloader):
            ...
            if not args.skip_save and step % 25 == 0 and step > 0:
                ...
                save_path_rank = save_tp_rank_checkpoint(
                    ...
                    optimizer=optimizer,
                    scheduler=scheduler,
                    training_state={"step": step, "epoch": epoch, "total_aux_loss": total_aux_loss},
                    extra_metadata={"source": "train_tp_moe.py", "step": step},
                )
                if rank == 0:
    ...
    if args.skip_save:
        if rank == 0:
            print("[Rank 0] Skipped checkpoint/model save (--skip_save)")
    else:
        ...
        save_path_rank = save_tp_rank_checkpoint(
            ...
            optimizer=optimizer,
            scheduler=scheduler,
            training_state={"step": max(step - 1, 0), "epoch": args.epochs - 1, "total_aux_loss": total_aux_loss, "final": True},
            extra_metadata={"source": "train_tp_moe.py", "final": True},
        )
        if rank == 0:
