#!/usr/bin/env python3
"""
Tensor Parallel training on Ascend NPU using transformer_engine (te)
Splits model layers across multiple NPUs
"""
...
try:
    from te import transformer_engine
    from te.transformer_engine.pytorch.attention import RotaryPositionalEmbedding
    TE_AVAILABLE = True
except ImportError:
    TE_AVAILABLE = False
    print("Warning: transformer_engine not available, using DDP instead")
...
def get_args():
    parser = argparse.ArgumentParser(description="TP Training on NPU")
    parser.add_argument("--model_name", type=str, default="/home/sd/npu_train/models/Qwen-Qwen2.5-7B-Instruct",
                        help="Model path")
    ...
    parser.add_argument("--save_path", type=str, default="./output_tp",
                        help="Model save path (default: ./output_tp)")
...
def train(args, local_rank, world_size, rank, device):
    ...
    print(f"[Rank {rank}] Starting TP training, tp_size={args.tp_size}, device={device}")
    ...
    # For TP, we need to use te's model conversion
    # This is a simplified approach - full TP requires specific model architecture
    if TE_AVAILABLE:
        try:
            # Try to enable mixed precision with te
            model = transformer_engine.pytorch.transformer.TransformerBlock(
                hidden_size=model.config.hidden_size,
                ffn_hidden_size=model.config.intermediate_size,
                num_attention_heads=model.config.num_attention_heads,
                num_layers=model.config.num_hidden_layers,
                layernorm_epsilon=model.config.layer_norm_eps,
                hidden_dropout=0.0,
                attention_dropout=0.0,
            )
            print(f"[Rank {rank}] Using transformer_engine for TP optimization")
        except Exception as e:
            print(f"[Rank {rank}] TE conversion failed, using standard model: {e}")
    ...
    if rank == 0:
        ...
        print(f"[Rank {rank}] TP size: {args.tp_size}")
    ...
    if rank == 0:
        print(f"\n{'='*50}")
        print(f"TP Training Configuration:")
